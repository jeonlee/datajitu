...
@section('content')
	...
	<div class="table">
	  <table class="middle">
	    ...
	    <tbody>
	    @foreach ($taiwn as $twan)

	      <tr>
		<td><div class="t_no f20">{{ Carbon\Carbon::parse($twan->tanggal)->format('d-m-Y') }}</div></td>
		<td><div class="t_hari f20">{{ $twan->periode }}-SG</div></td>

		<td><div class="t_tgl biru f20"><a class="blue-number" href="/analisis/{{ $twan->id }}">{{ $twan->number }}</a></div></td>   
